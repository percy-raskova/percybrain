...
describe("PercyBrain Core", function()
  ...
  after_each(function()
    -- No cleanup needed - configuration remains loaded
  end)

  it("loads configuration without errors", function()
    -- Arrange: No setup needed

    -- Act: Attempt to load config
    local config_ok = pcall(require, "config")

    -- Assert: Should load successfully
    assert.is_true(config_ok)
  end)

  it("loads exactly 81 plugins", function()
    -- Arrange: Ensure lazy.nvim is available
    local ok, lazy = pcall(require, "lazy")

    -- Act & Assert: Count plugins if lazy loaded
    if ok then
