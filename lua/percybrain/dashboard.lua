...
M.toggle = function()
  ...
  table.insert(
    lines,
    string.format("  📝 Total Notes:       %d", metrics.link_density.total_notes)
  )
  table.insert(
    lines,
    string.format("  🔗 Total Links:       %d", metrics.link_density.total_links)
  )
  table.insert(
    lines,
    string.format("  📊 Average Density:   %.2f links/note", metrics.link_density.avg_density)
  )
  table.insert(
    lines,
    string.format("  ⬢  Hub Notes:         %d (5+ links)", metrics.link_density.hub_notes)
  )
  table.insert(
    lines,
    string.format(
