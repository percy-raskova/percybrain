...
**Date**: 2025-10-18
**Design Document**: `AI_DASHBOARD_TEST_DESIGN.md` (full 5,600-word specification)
...
**Test Suite Scope**: 40 tests across 8 describe blocks
**Estimated Test File Size**: 550-650 lines
**Expected Pass Rate**: 85-95% (30-35/40 tests)
...
**Challenge**: Dashboard analyzes real filesystem (markdown files, frontmatter, wiki links)
**Solution**: Mock `vim.fn.systemlist()` and `io.open()` with virtual file system
...
**Challenge**: Growth analysis uses Unix timestamps and 7-day windows
**Solution**: Freeze time with mock controller for deterministic tests
...
**Challenge**: Ollama API calls via `curl` system commands
**Solution**: Mock `vim.fn.system()` to return JSON responses
...
**Challenge**: 5-minute cache with time-based expiration
**Solution**: Combine time mocking with call counting
...
**Challenge**: Floating window with complex buffer formatting
**Solution**: Capture nvim_buf_set_lines calls and validate content structure
